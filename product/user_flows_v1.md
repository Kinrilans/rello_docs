...
Цель: зафиксировать **полное пользовательское поведение** по всем функциям: кто, где (какой бот), с какими правами, по каким шагам и какие системы задействованы. Это «якорь» для разработки и тест‑кейсов.
...
> Сети/токены: **TRC‑20**, **USDT/USDC**. Роли: **TRADER / APPROVER / COMPANY_ADMIN / ARBITER / OPS / PLATFORM_ADMIN**. Смежные документы: *Таблица tier’ов (v2), Модуль депозитов и лимитов (v1), Engine авто‑выплат (v1), Deeplink/JWT (v1), API‑контракты (v1), События/вебхуки (v1), Мониторинг и алерты (v1)*.
